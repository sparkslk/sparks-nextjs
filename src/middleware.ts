...
export async function middleware(req: NextRequest) {
  ...
  const roleRoutes: Record<string, RegExp> = {
    ...
    THERAPIST: /^\/therapist\/(dashboard|verification|profile(\/setup|\/edit|\/change-password)?|patients(\/new|\/[^\/]+)?|sessions(\/[^\/]+)?|setAvailability|assessments(\/new|\/[^\/]+)?|messages|blogs(\/new|\/[^\/]+|\/[^\/]+\/edit)?|appointments(\/new)?|assignment-requests|requests)/,
