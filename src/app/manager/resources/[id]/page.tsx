...
export default function ManagerResourceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleConfirmDelete = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/manager/resources/${blog.id}`, {
        method: "DELETE",
      });
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <div className="flex items-center mt-3">
              <div className="w-8 h-8 rounded-full bg-gray-500 text-white text-sm flex items-center justify-center font-medium">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/manager/resources/${blog.id}/edit`)}
              className="transition-all duration-300 hover:bg-[#F5F3FB]"
            >
