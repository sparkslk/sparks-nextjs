...
export async function POST(req: NextRequest) {
    try {
        ...
        const requestedDateOnly = new Date(requestedDateTime.toDateString()); // Normalize to date only
        ...
        if (availabilitySlots.length === 0) {
            return NextResponse.json({
                available: false,
                message: "Therapist has not set their availability for this date",
                suggestedSlots: []
            });
        }
        ...
        if (!isAvailable) {
            return NextResponse.json({
                ...
                suggestedSlots: getSuggestedSlots(availabilitySlots)
            });
        }
...
// Helper function to get suggested time slots from availability records
function getSuggestedSlots(availability: Array<{
    id: string;
    therapistId: string;
    date: Date;
    startTime: string;
