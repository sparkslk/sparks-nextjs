...
export default function ParentDashboard() {
    ...
    // const [selectedTimeSlotIdx, setSelectedTimeSlotIdx] = useState(1);
                        ...
                        <Card className="shadow-md border-border bg-background">
                            ...
                            <CardContent className="pt-6">
                                <div className="space-y-4">
                                    {parentData.children.map(child => {
                                        const popup = popupState[child.id] || {selectedDate: new Date(), selectedSlot: null, showCalendar: false};
                                        const year = popup.selectedDate.getFullYear();
                                        const month = popup.selectedDate.getMonth();
                                        ...
                                        return (
                                            <div key={child.id} className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4 border rounded-lg bg-accent/10 border-accent">
                                                ...
                                                <div className="flex gap-2 items-center">
                                                    ...
                                                    {child.therapist ? (
                                                        <Button
                                                            ...
                                                            onClick={() => openBookSession(child.id)}
                                                        >
                                                            Book a Session
                                                        </Button>
                                                    ) : (
                                                ...
                                                {/* Book Session Modal */}
                                                <Dialog open={showBookSession === child.id} onOpenChange={open => open ? openBookSession(child.id) : closeBookSession()}>
                                                    <DialogContent className="max-w-2xl w-full p-0 sm:p-0 overflow-y-auto max-h-[90vh] rounded-2xl shadow-2xl border border-border">
                                                        <div className="bg-background rounded-2xl p-0 sm:p-0">
                                                            ...
                                                            <div className="px-8 pt-4 pb-2">
                                                                ...
                                                                <div className="flex gap-2 mb-2 justify-start flex-wrap">
                                                                    {Array.from({ length: 5 }).map((_, i) => {
                                                                        ...
                                                                        const isSelected = d.toDateString() === popup.selectedDate.toDateString();
                                                                        return (
                                                                            <button
                                                                                key={i}
                                                                                ...
                                                                                type="button"
                                                                                onClick={() => !isPast && handleDateClick(child.id, d.getDate())}
                                                                                disabled={isPast}
                                                                            >
                                                                ...
                                                                <div className="flex justify-start mb-4">
                                                                    <Button variant="outline" size="sm" className="text-xs px-3 py-1" onClick={() => toggleCalendar(child.id)}>
                                                                        {popup.showCalendar ? "Hide Calendar" : "View Calendar"}
                                                                    </Button>
                                                                </div>
                                                                {popup.showCalendar && (
                                                                    <div className="bg-gradient-to-br from-muted/40 to-background rounded-2xl p-6 border border-border mb-2 shadow-md w-full max-w-lg mx-auto">
                                                                        <div className="flex justify-between items-center mb-4">
                                                                            <span className="font-semibold text-2xl text-primary">{popup.selectedDate.toLocaleString('default', { month: 'long' })} {year}</span>
                                                                        </div>
                                                                        ...
                                                                        <div className="grid grid-cols-7 gap-2 border border-border rounded-xl bg-card p-2">
                                                                            ...
                                                                            {Array.from({ length: daysInMonth }).map((_, i) => {
                                                                                ...
                                                                                const isSelected = popup.selectedDate.getDate() === day && popup.selectedDate.getMonth() === month;
                                                                                return (
                                                                                    <button
                                                                                        key={day}
                                                                                        ...
                                                                                        onClick={() => !isPast && handleDateClick(child.id, day)}
                                                            ...
                                                            {/* Time Slots */}
                                                            <div className="px-8 pt-4 pb-2">
                                                                <div className="font-semibold text-sm text-muted-foreground mb-2">Available Time Slots <span className="text-primary">{popup.selectedDate && popup.selectedDate.toLocaleString('default', { month: 'short', day: 'numeric' })}</span></div>
                                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-2">
                                                                    {timeSlots.map(slot => (
                                                                        <Button
                                                                            key={slot}
                                                                            variant={popup.selectedSlot === slot ? "default" : "outline"}
                                                                            className={`rounded-xl px-4 py-2 text-base font-semibold shadow-sm ${popup.selectedSlot === slot ? "bg-primary text-white" : ""}`}
                                                                            onClick={() => handleSlotClick(child.id, slot)}
                                                                        >
                                                                            {slot}
                                                                        </Button>
                                                                    ))}
                                                            ...
                                                            <div className="px-8 pt-4 pb-8">
                                                                ...
                                                                <div className="bg-muted rounded-xl p-6 border border-border shadow-sm">
                                                                    ...
                                                                    <div className="flex justify-between text-sm mb-2 text-muted-foreground">
                                                                        <span>Date:</span> <span className="font-medium text-foreground">{weekdays[popup.selectedDate.getDay()]} {popup.selectedDate.getDate()}</span>
                                                                    </div>
                                                                    <div className="flex justify-between text-sm mb-2 text-muted-foreground">
                                                                        <span>Time:</span> <span className="font-medium text-foreground">{popup.selectedSlot || "--"}</span>
                                                                    </div>
                                                                    <div className="flex justify-between text-sm mb-2 text-muted-foreground">
                                                                    ...
                                                                    <Button className="w-full bg-primary text-white text-base py-3 rounded-xl font-semibold shadow" disabled={!popup.selectedSlot || bookingConfirmed[child.id]} onClick={() => handleConfirmBooking(child.id)}>
                                                                        {bookingConfirmed[child.id] ? "Booking Confirmed!" : "Confirm Booking"}
                                                                    </Button>
                                                                    {bookingConfirmed[child.id] && (
